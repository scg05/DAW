...
$fotos_sql = "SELECT IdFoto, Foto, Titulo FROM Fotos WHERE Anuncio = ?";
...
while ($row = $fotos_result->fetch_assoc()) {
    // Cada elemento será un array con IdFoto, Foto, Titulo
    $miniaturas[] = $row;
}
...
<main class="container">
  ...
  <article class="detalle-anuncio">
    ...
    <!-- Miniaturas -->
    <?php 
echo "<p>Miniaturas encontradas: " . count($miniaturas) . "</p>";

if (!empty($miniaturas)): ?>
      <div class="miniaturas">
        <?php foreach ($miniaturas as $f): ?>
          <div class="miniatura-item">
            <img src="uploads/anuncios/<?= htmlspecialchars($f['Foto'], ENT_QUOTES, 'UTF-8'); ?>" 
                alt="<?= htmlspecialchars($f['Titulo'] ?? 'Foto anuncio', ENT_QUOTES, 'UTF-8'); ?>" 
                width="100" class="mini">
            <br>
            <a href="respuesta_eliminar_foto.php?id=<?= $f['IdFoto']; ?>">
              Eliminar esta foto
            </a>
          </div>
        <?php endforeach; ?>
    ...
    <!-- Eliminar anuncio -->
     <p>
      <a href="respuesta_eliminar_anuncio.php?id=<?= $anuncio['IdAnuncio']; ?>">
    ...
    <!-- Modificar anuncio -->
     <p>
      <a href="modificar_anuncio.php?id=<?= $anuncio['IdAnuncio']; ?>">Modificar este anuncio</a>
    </p>
